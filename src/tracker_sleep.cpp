...
TrackerSleepResult TrackerSleep::sleep() {
  ...
  if (_nextWakeMs == 0) {
    Log.trace("cancelled sleep because of first update");
    cancelSleep = true;
  }
  if (_nextWakeMs < now) {
  ...
  if (duration < ((system_tick_t)_config_state.connecting_max_seconds * 1000)) {
    Log.trace("cancelled sleep at %lu milliseconds because too soon before wake", (uint32_t)_nextWakeMs);
    cancelSleep = true;
  }
  ...
  duration -= (_config_state.connecting_max_seconds * 1000);
...
int TrackerSleep::loop() {
  ...
  switch (_executionState) {
    ...
    case TrackerExecutionState::EXECUTION: {
      ...
      if (_config_state.mode != TrackerSleepMode::Disable) {
        ...
        if (!_holdSleep &&
            (System.uptime() - _lastExecuteSec >= (uint32_t)_config_state.execute_min_seconds)) {
